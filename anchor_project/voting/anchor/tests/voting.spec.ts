...
describe('voting', () => {
  ...
  it('Initialize candidate', async () => {
    await program.methods
      .initializeCandidate("Suhana", new anchor.BN(pollId), "RGUKT", "", "")
      .signers([payer.payer])
      .rpc()

    const [candidateAddress] = PublicKey.findProgramAddressSync(
      [new anchor.BN(pollId).toBuffer("le", 8), Buffer.from("Suhana")],
      program.programId
    )
    ...
    expect(candidateInfo.candidateName).toEqual("Suhana");
    expect(candidateInfo.poll).toEqual(pollAddress);
  })

  it('Voting', async () => {
    await program.methods
      .vote("Suhana", new anchor.BN(pollId))
      .accounts({
        mint: mint,
